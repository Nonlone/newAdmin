...
public class AdvertBlockController extends BaseListableController<AdvertBlock> {
	...
	public Object list(@ModelAttribute QueryRequest queryRequest) {
		PageInfo pageInfo = advertBlockService.list(queryRequest.getPageIndex(), queryRequest.getLimit(),
				queryRequest.getBlockId(), queryRequest.getStartTime(), queryRequest.getEndTime(),
				queryRequest.getStatus(), queryRequest.getField(), queryRequest.getDirection());
	...
	public Object getGroupItems() {
		...
		groupList.forEach(advertGroup -> {
			result.add(new SelectItem(advertGroup.getTitle(), String.valueOf(advertGroup.getId())));
		});
		return result;
	}
