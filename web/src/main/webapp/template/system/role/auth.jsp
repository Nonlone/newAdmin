...
<script type="text/javascript">
    BUI.use(['bui/tree','bui/data','bui/toolbar','bui/editor','bui/select','bui/ux/crudgrid','bui/mask','bui/overlay'],
        function (Tree,Data,Toolbar,Editor,Select,CrudGrid,Mask) {
            ...
            //加载权限键值对
            var enumPermission = {};
            $.ajax({url:'${ctx}/system/permission/permissionCheckList',
                async : false,
                dataType : 'json',
            ...
            var loadRole = function(){
                //加载并在权限数上打勾
                $.ajax({ url :'${ctx}/system/role/update/'+roleId,
                    cache : false,
                    dataType : 'json',
                    success : function(role){
                        ...
                        if(roleAuths){
                            $.each(roleAuths,function(index, roleAuth){
                                var node = tree.getItem(roleAuth.resourceId);
                                if(node.leaf){
                                    tree.setChecked(tree.getItem(roleAuth.resourceId));
                                    var permissionIds = roleAuth.permissionIds;
                                    if (permissionIds) {
                                        permissionIds = permissionIds.toString().split(',');
                                        var checkboxs = $('#'+roleAuth.resourceId+'>input[type=\'checkbox\']');
