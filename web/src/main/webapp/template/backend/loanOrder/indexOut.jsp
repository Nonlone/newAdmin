...
function openout(id) {
    window.open('${IP}'+'${ctx}/admin/order/loanOrder/auth/'+id);
}
                    ...
                    if(result.code=="SUC000"){
                        ...
                        BUI.Message.Alert('操作成功！',function(){
                            $.post('${ctx}/admin/order/loanOrder/stopLoan/'+id,function(result){
                                if(result.success){
                                    BUI.Message.Alert('已提交终止放款请求！',function(){
                                    },'success');
                                }else{
...
BUI.use(['bui/ux/crudgrid','bui/common/search','bui/common/page','bui/overlay'],function (CrudGrid,Search,Grid,Overlay) {
	...
	<framwork:crudPermission resource="/admin/order/loanOrder"/>

    var authUrl,authBtn=false;//授权按钮
		authBtn = true;
		authUrl = '${ctx}/admin/order/loanOrder/auth/';
	...
	var crudGrid = new CrudGrid({
		entityName : '借款订单表',
    	pkColumn : 'id',//主键
      	storeUrl : '${ctx}/admin/order/loanOrder/listOut',
        addUrl : '${ctx}/admin/order/loanOrder/add',
        updateUrl : '${ctx}/admin/order/loanOrder/update',
        removeUrl : '${ctx}/admin/order/loanOrder/del',
        columns : columns,
        showAddBtn : add,
			...
			if(authBtn){
			 ...
			 if(obj.status=="40"||obj.status=="20"){
			     if(obj.cancelLoan==null){
                     editStr= detail+'<span class="x-icon x-icon-error" title="终止放款" onclick="stop(\'\'+id+\'\');"><i class="icon icon-white icon-ban-circle"></i></span>';
