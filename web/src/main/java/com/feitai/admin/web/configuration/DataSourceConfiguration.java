...
public class DataSourceConfiguration implements EnvironmentAware {
	...
	public Map<String, String> classPrefixMap() {
		...
		// 管理后台广告
		classPrefixMap.put("com.feitai.admin.mop.appversion", MOP_DATASOURCE);
		...
		if (!CollectionUtils.isEmpty(classPrefixMap)) {
			try {
				for (Map.Entry<String, String> entry : classPrefixMap.entrySet()) {
					DataSource dataSource = getDataSource(entry.getValue(), DEFAULT_PROPERTIES_PREFIX,
				...
				if (multipleDataSourceMap.containsKey(ADMIN_DATASOURCE)) {
					return new MultipleDataSource(multipleDataSourceMap,
							(DataSource) multipleDataSourceMap.get(ADMIN_DATASOURCE));
				} else {
	 ...
	 */
	@Bean
	public ClassPrefixMultiDataSourceInterceptor classPrefixMultiDataSourceInterceptor(
			@Autowired MultipleDataSource multipleDataSource,
			@Qualifier(BEAN_CLASS_PREFIX_MAP) Map<String, String> classPrefixMap) throws SQLException {
		...
		for (String key : dataSourceKeySet) {
			connectionSignatureMap.put(key,
					new ConnectionSignature(multipleDataSource.getDataSouce(key).getConnection()));
		}
	 ...
	 */
	@Bean
	public ClassPrefixMultiDataSourceSelector classPrefixMultiDataSourceSelector(
			@Autowired ClassPrefixMultiDataSourceInterceptor classPrefixMultiDataSourceInterceptor,
			@Autowired MultipleDataSource multipleDataSource) {
	...
	public Interceptor[] mybatisInterceptors(
			@Autowired ClassPrefixMultiDataSourceInterceptor classPrefixMultiDataSourceInterceptor) {
		return new Interceptor[] { classPrefixMultiDataSourceInterceptor };
	}
	 ...
	 * @throws Exception
	 */
	private DataSource getDataSource(String propertiesPrefix, String defaultPropertiesPrefix,
			String connectionProperties) throws Exception {
		...
		props.put(DruidDataSourceFactory.PROP_DRIVERCLASSNAME,
				environment.getProperty(defaultPropertiesPrefix + ".driver"));
		// Config optimize
		props.put(DruidDataSourceFactory.PROP_VALIDATIONQUERY,
				environment.getProperty(defaultPropertiesPrefix + ".validationQuery"));
		props.put(DruidDataSourceFactory.PROP_MAXACTIVE,
				environment.getProperty(defaultPropertiesPrefix + ".poolMaximumActiveConnections"));
		props.put(DruidDataSourceFactory.PROP_MAXIDLE,
				environment.getProperty(defaultPropertiesPrefix + ".poolMaximumIdleConnections"));
		props.put(DruidDataSourceFactory.PROP_INITIALSIZE,
				environment.getProperty(defaultPropertiesPrefix + ".initialSize"));
		props.put(DruidDataSourceFactory.PROP_FILTERS, environment.getProperty(defaultPropertiesPrefix + ".filters"));
		props.put(DruidDataSourceFactory.PROP_TIMEBETWEENEVICTIONRUNSMILLIS,
				environment.getProperty(defaultPropertiesPrefix + ".timeBetweenEvictionRunsMillis"));
		props.put(DruidDataSourceFactory.PROP_MINEVICTABLEIDLETIMEMILLIS,
				environment.getProperty(defaultPropertiesPrefix + ".minEvictableIdleTimeMillis"));
		props.put(DruidDataSourceFactory.PROP_TESTONBORROW,
				environment.getProperty(defaultPropertiesPrefix + ".testOnBorrow"));
		props.put(DruidDataSourceFactory.PROP_TESTONRETURN,
				environment.getProperty(defaultPropertiesPrefix + ".testOnReturn"));
		props.put(DruidDataSourceFactory.PROP_POOLPREPAREDSTATEMENTS,
				environment.getProperty(defaultPropertiesPrefix + ".poolPreparedStatements"));
		props.put(DruidDataSourceFactory.PROP_MAXOPENPREPAREDSTATEMENTS,
				environment.getProperty(defaultPropertiesPrefix + ".maxPoolPreparedStatementPerConnectionSize"));
		props.put(DruidDataSourceFactory.PROP_MAXWAIT, environment.getProperty(defaultPropertiesPrefix + ".maxWait"));
		props.put(DruidDataSourceFactory.PROP_REMOVEABANDONEDTIMEOUT,
				environment.getProperty(defaultPropertiesPrefix + ".removeAbandonedTimeout"));
